...
public class TemperatureEndpoint {
	private static final String namespaceUri = "http://arisromil.com/weather/schemas";
	...
	public Element getTemperature(@RequestPayload Element requestElement) {
		...
		var dates = datePath.selectNodes(requestElement).stream().map(Node::getText)
				.map(ds -> LocalDate.parse(ds, DateTimeFormatter.ISO_DATE)).toList();
		...
		var responseElement = responseDocument.addElement("GetTemperaturesResponse", namespaceUri);
