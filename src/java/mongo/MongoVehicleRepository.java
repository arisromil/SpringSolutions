...
public class MongoVehicleRepository implements VehicleRepository {
	...
	public List<Vehicle> findAll() {
		return StreamSupport.stream(getCollection().find().spliterator(), false).map(this::transform)
	...
	private Vehicle transform(Document dbVehicle) {
		...
		return new Vehicle(dbVehicle.getString("vehicleNo"), dbVehicle.getString("color"),
				dbVehicle.getInteger("wheel"), dbVehicle.getInteger("seat"));
	}

	private Document transform(Vehicle vehicle) {
		return new Document("vehicleNo", vehicle.vehicleNo()).append("color", vehicle.color())
				.append("wheel", vehicle.wheel()).append("seat", vehicle.seat());
	}
}