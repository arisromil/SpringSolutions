...
public class IntegrationConfiguration {
	...
	public IntegrationFlow inboundFileFlow(@Value("${user.home}/inboundFiles/new/") File directory) {
		return IntegrationFlow.from(Files.inboundAdapter(directory).patternFilter("*.csv"),
				c -> c.poller(Pollers.fixedRate(Duration.ofSeconds(10)))).handle(messageProcessor()).get();
	}
